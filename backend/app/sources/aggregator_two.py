...
class AggregatorTwoSource(FlightSource):
    def __init__(self) -> None:
        self.config = SourceRuntimeConfig(
            name="aggregator_two",
            enabled=settings.source_aggregator_two_enabled,
            timeout_seconds=20,
            max_retries=2,
            backoff_ms=300,
    ...
    def _fixture_candidates(self) -> list[RawItineraryCandidate]:
        path = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "sources" / "aggregator_two_candidates.json"
